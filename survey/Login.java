...
public class Login {
	...
	public void loginView() throws SQLException {
		...
		login.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(username.isEmpty() || password.isEmpty()) {
					JOptionPane.showMessageDialog(frame, "Please Enter Username and Password both", "Caution", JOptionPane.WARNING_MESSAGE);
				}
				else {
					try {
						SQLManage manage= new SQLManage();
					...
					if (id == -1) {
						JOptionPane.showMessageDialog(frame, "No such User", "Caution", JOptionPane.WARNING_MESSAGE);
					}
					else if(id == 0) {
						JOptionPane.showMessageDialog(frame, "Incorrect Password", "Caution", JOptionPane.WARNING_MESSAGE);
					}
					else {
		...
		JButton attend = new JButton("ATTEND A SURVEY (FOR GUESTS)");
				...
				try {
					if(!surveyCode.isEmpty() && surveyCode.length() == 5) {
						if(manage.check(surveyCode)) {
							...
							JOptionPane.showMessageDialog(frame, "No such Survey", "Caution", JOptionPane.WARNING_MESSAGE);
						}
					}
				}
				catch(Exception e2) {
